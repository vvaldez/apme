...
These rules are generated by the deprecation pipeline and detect patterns deprecated in upcoming ansible-core releases.

| Rule ID | Validator | Description | Overlap |
|---------|-----------|-------------|---------|
| M014 | native | Top-level ansible_* fact variables; use `ansible_facts['name']` | L076 |
| M015 | native | `play_hosts` magic variable; use `ansible_play_batch` | — |
| M016 | OPA | Empty `when:` on task is deprecated | — |
| M017 | OPA | `action:` key with mapping value deprecated | — |
| M018 | OPA | Paramiko connection plugin deprecated | — |
...
| M023 | OPA | String booleans for `follow_redirects` deprecated | L061 |
| M024 | OPA | `include_vars` `ignore_files` deprecated | — |
| M025 | OPA | Third-party strategy plugins deprecated | — |
| M026 | native | Invalid inventory variable names | L050 |
| M027 | native | Legacy `k=v` args merged with `args:` mapping | L046, L062 |
| M028 | OPA | `first_found` auto-split on colon/comma deprecated | — |
| M029 | native | Inventory script missing `_meta` key (disabled) | — |
