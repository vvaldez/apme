"""GraphRule R101: detect parameterized command execution."""
...
@dataclass
class CommandExecGraphRule(GraphRule):
    """Flag tasks that execute a command containing Jinja-templated values.

    A templated command string means the actual command executed depends on
    variable values, which is a supply-chain risk when variables are
    ...
    def process(self, graph: ContentGraph, node_id: str) -> GraphRuleResult | None:
        """Flag when the command string contains Jinja2 template syntax.
        ...
        cmd = resolve_field(mo, profile, "cmd")

        if not cmd or not is_templated(cmd):
