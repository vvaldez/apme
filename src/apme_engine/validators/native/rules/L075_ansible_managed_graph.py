"""GraphRule L075: Template ``src`` should use a ``.j2`` extension (ansible_managed)."""
...
@dataclass
class AnsibleManagedGraphRule(GraphRule):
    """Flag static template ``src`` values that omit a ``.j2`` suffix.
    ...
    def process(self, graph: ContentGraph, node_id: str) -> GraphRuleResult | None:
        ...
        Returns:
            ...
            string without Jinja and not ending in ``.j2``, ``verdict`` False
            otherwise, or None if the node is missing.
        """
        node = graph.get_node(node_id)
        if node is None:
            return None
